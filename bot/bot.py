...
class BotApp(object):
    ...
    def __init__(self, net, sess):
        relevance = -1
        self.relevance = relevance
        self.states = initial_state_with_relevance_masking(net, sess, relevance)

    def chatbot(self, net, sess, chars, vocab, message):

        relevance, temperature, beam_width = -1, 1, 2
        user_input = sanitize_text(vocab, message)
        user_command_entered, reset, states, relevance, temperature, beam_width = process_user_command(user_input, self.states, relevance, temperature, beam_width)
        if reset:
        ...
        for i, char_token in enumerate(computer_response_generator):
            answer += chars[char_token]
            states = forward_text(net, sess, states, vocab, chars[char_token])
        self.states = forward_text(net, sess, states, vocab, '\n> ')
